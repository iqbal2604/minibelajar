...
type TicketQuery struct {
	...
	withTechnicians *TechnicianQuery
...
// QueryTechnicians chains the current query on the "technicians" edge.
func (_q *TicketQuery) QueryTechnicians() *TechnicianQuery {
	query := (&TechnicianClient{config: _q.config}).Query()
...
func (_q *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withTechnicians: _q.withTechnicians.Clone(),
...
// WithTechnicians tells the query-builder to eager-load the nodes that are connected to
// the "technicians" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TicketQuery) WithTechnicians(opts ...func(*TechnicianQuery)) *TicketQuery {
	query := (&TechnicianClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTechnicians = query
	return _q
}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withTechnicians; query != nil {
		if err := _q.loadTechnicians(ctx, query, nodes,
			func(n *Ticket) { n.Edges.Technicians = []*Technician{} },
			func(n *Ticket, e *Technician) { n.Edges.Technicians = append(n.Edges.Technicians, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TicketQuery) loadTechnicians(ctx context.Context, query *TechnicianQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Technician)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(ticket.TechniciansTable)
		s.Join(joinT).On(s.C(technician.FieldID), joinT.C(ticket.TechniciansPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(ticket.TechniciansPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(ticket.TechniciansPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Technician](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "technicians" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
