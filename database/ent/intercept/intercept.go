...
// The TechnicianFunc type is an adapter to allow the use of ordinary function as a Querier.
type TechnicianFunc func(context.Context, *ent.TechnicianQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f TechnicianFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.TechnicianQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.TechnicianQuery", q)
}

// The TraverseTechnician type is an adapter to allow the use of ordinary function as Traverser.
type TraverseTechnician func(context.Context, *ent.TechnicianQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseTechnician) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseTechnician) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TechnicianQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.TechnicianQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.TechnicianQuery:
		return &query[*ent.TechnicianQuery, predicate.Technician, technician.OrderOption]{typ: ent.TypeTechnician, tq: q}, nil
